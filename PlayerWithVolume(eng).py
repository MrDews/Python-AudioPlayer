...
def browse_button():
    ...
    for image in sorted(
        os.listdir(filename)
    ):  # A loop that outputs music to the screen
        if (
            (image.endswith(".mp3"))
...
def play():
    ...
    if (i > count - 1) or (i < 0):  # Condition to avoid leaving the list value area
        ...
        p = filename + "/" + s[i]  # Variable, for simplified playback operation
...
def pause():
    ...
    if (
        pause == False
    ):  # Checking if the music is turned off, the pause does not work (it seems to work and it seems not)
        pause = True
        rightNow = 0
        mixer.music.pause()
